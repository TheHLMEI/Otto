...
ottolog_init(char *logdir, char *logfile, int level, int truncate)
{
   ...
   if(logfile != NULL && logfile[0] != '\0')
   {
      ...
      else
      {
         ...
         {
            otto_sprintf(fname, "%s/%s%s", logdir, logfile, extension);
...
vlprintf(OTTOLOG *logp, int level, char *format, va_list ap)
{
   ...
   otto_sprintf(buffer, "%8.8s %s: ", &fulldate[11], levelstr(level));
...
vlsprintf(OTTOLOG *logp, int level, char *format, va_list ap)
{
      ...
      level != ENDI)
   {
      ...
      otto_sprintf(logp->logfile_buffer, "%8.8s %s: ", &fulldate[11], levelstr(level));
