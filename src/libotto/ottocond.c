...
int
compile_expression(char *output, DBCTX *ctx, char *condition, int outlen)
{
         ...
         case 't':
            *t++ = *s++;
            ...
            if(l > 2)
            {
               ...
               index = find_jobname(ctx, name);
               // lprintf(logp, INFO, "Lookup name = '%s' index = %d id = %d\n", name, index, ctx->job[index].id);
               if(index != -1)
               {
                  index = ctx->job[index].id;
               }
               else
               {
                  retval = OTTO_FAIL;
...
int
validate_dependencies(DBCTX *ctx, char *name, char *condition)
{
   int retval = 0;
