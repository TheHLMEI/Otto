...
static int last_worksort = BY_UNKNOWN;
...
void  sort_jobwork_by_linked_list();
void  add_job_to_tmpwork(JOB *tmpwork, int place, int id);
...
copy_jobwork()
{
	...
	memcpy(jobwork, job, sizeof(JOB)*cfg.ottodb_maxjobs);
	last_worksort = BY_UNKNOWN;
}



void
sort_jobwork(int sort_mode)
{
	last_worksort = sort_mode;

	switch(sort_mode)
	{
		case BY_ACTIVE:      qsort(jobwork, cfg.ottodb_maxjobs, sizeof(JOB), cmp_job_by_active);    break;
		case BY_ID:          qsort(jobwork, cfg.ottodb_maxjobs, sizeof(JOB), cmp_job_by_id);        break;
		case BY_LINKED_LIST: sort_jobwork_by_linked_list();                                         break;
		case BY_NAME:        qsort(jobwork, cfg.ottodb_maxjobs, sizeof(JOB), cmp_job_by_name);      break;
		case BY_PID:         qsort(jobwork, cfg.ottodb_maxjobs, sizeof(JOB), cmp_job_by_pid);       break;
		case BY_DATE_COND:   qsort(jobwork, cfg.ottodb_maxjobs, sizeof(JOB), cmp_job_by_date_cond); break;
		default:             last_worksort = BY_UNKNOWN; break;
	}
}
...
sort_jobwork_by_linked_list()
{
	...
	add_job_to_tmpwork(tmpwork, 0, root_job->head);

	memcpy(jobwork, tmpwork, sizeof(JOB)*cfg.ottodb_maxjobs);
...
void
add_job_to_tmpwork(JOB *tmpwork, int place, int id)
{
	while(id != -1)
	{
		memcpy(&tmpwork[place++], &jobwork[id], sizeof(JOB));
		if(jobwork[id].type == OTTO_BOX)
			add_job_to_tmpwork(tmpwork, place, jobwork[id].head);
...
{
	sort_jobwork(BY_ID);
	memcpy(job, jobwork, sizeof(JOB)*cfg.ottodb_maxjobs);
...
int
find_jobname(char *jobname)
{
	int i;
	int retval = -1;

	if(last_worksort != BY_NAME)
	...
	for(i=0; i<cfg.ottodb_maxjobs; i++)
	{
		...
		{
			if(strcmp(jobname, jobwork[i].name) == 0)
			{
				retval = i;
				break;
			}
...
void
clear_jobwork(int job_index)
{
	int j;

	j = jobwork[job_index].id;
	memset(&jobwork[job_index], 0, sizeof(JOB));
	jobwork[job_index].id = j;
}
