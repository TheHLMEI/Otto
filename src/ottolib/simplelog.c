...
int vlprintf(SIMPLELOG *logp, int level, char *format, va_list ap);
int vlsprintf(SIMPLELOG *logp, int level, char *format, va_list ap);



SIMPLELOG *
simplelog_init(char *logdir, char *logfile, int level, int truncate)
{
	SIMPLELOG *retval = NULL;
	int  i;
	char fname[PATH_MAX], *mode, *extension;

	if(logfile != NULL && logfile[0] != '\0')
	{
		if((retval = (SIMPLELOG *)malloc(sizeof(SIMPLELOG))) == NULL)
		{
			// Failure of memory allocation is fatal.
			fprintf(stderr, "In simplelog_init(), malloc() failed for log file '%s'. [%d]\n", logfile, errno);
			perror("Error");
		}
		else
		{
...
int
lprintf(SIMPLELOG *logp, int level, char *format, ...)
{
	va_list ap;
	int     retval;
...
int
vlprintf(SIMPLELOG *logp, int level, char *format, va_list ap)
{
	char   buffer[SIMPLELOG_BUFLEN];
	int    length;
	time_t now;
	int    retval = 0;
...
int
lsprintf(SIMPLELOG *logp, int level, char *format, ...)
{
	va_list ap;
	int     retval;
...
int
vlsprintf(SIMPLELOG *logp, int level, char *format, va_list ap)
{
	int    length;
	time_t now;
	int    retval = 0;
	...
	if(level != CAT  &&
		...
		level != ENDI)
	{
		if(logp->lsstarted == SIMPLELOG_TRUE)
			lprintf(logp, INFO, "lsprintf restarted!\n");
		logp->lsstarted = SIMPLELOG_TRUE;
		logp->lslevel = level;
	...
	if(logp->lsstarted == SIMPLELOG_TRUE)
	{
		...
		if(level == END || level == ENDI)
		{
			...
			logp->lsstarted = SIMPLELOG_FALSE;
		}
	}
...
void
lperror(SIMPLELOG *logp, int level, char *string)
{
	int add_colon;
